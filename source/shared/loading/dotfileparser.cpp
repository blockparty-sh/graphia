...
const x3::rule<class L, List> dotList = "list";
...
const auto gmlValue_def = double_ | int_ | noQuotesString | (x3::lit('[') >> dotList >> x3::lit(']'));
...
const auto dotList_def = *gmlKeyValue;

const x3::rule<class L, List> dot = "dot";
const auto dot_def = *gmlKeyValue;

BOOST_SPIRIT_DEFINE(dotList, noQuotesString, gmlKey, gmlValue, gmlKeyValue)
...
bool build(DotFileParser& parser, const List& gml, IGraphModel& graphModel,
...
bool DotFileParser::parse(const QUrl& url, IGraphModel* graphModel)
{
    ...
    SpiritDotParser::List dot;
    bool success = false;

    try
    {
        success = SpiritDotParser::x3::phrase_parse(it, end,
            SpiritDotParser::dotList, SpiritDotParser::ascii::space, dot);
    }
